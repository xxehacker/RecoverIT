...
export default function ClaimRequestForm() {
  ...
  const handleFileChange = (e) => {
    ...
    // In a real app, you would process files, maybe upload them, and store URLs
    setFormData({
      ...formData,
                ...
                {["app", "phone", "email"].map((method) => (
                  <label key={method} className="inline-flex items-center">
                    <input
                      ...
                      value={method}
                      checked={formData.contactInformation.preferredContactMethod === method}
                      onChange={(e) => handleInputChange(e, "contactInformation")}
  ...
  const renderReviewSection = () => {
    return (
      <div className="bg-white p-6 rounded-lg shadow-sm">
        <h3 className="text-xl font-bold text-gray-800 mb-4">Claim Request Summary</h3>
        
        <div className="space-y-4">
          <div>
            <h4 className="text-md font-semibold text-gray-700">Item Description</h4>
            <p className="text-gray-600 mt-1">{formData.description || "Not provided"}</p>
          </div>
          
          <div>
            <h4 className="text-md font-semibold text-gray-700">Item Identifiers</h4>
            <div className="ml-4 mt-1">
              <p className="text-gray-600"><span className="font-medium">Serial Number:</span> {formData.itemIdentifiers.serialNumber || "Not provided"}</p>
              <p className="text-gray-600"><span className="font-medium">Unique Marks:</span> {formData.itemIdentifiers.uniqueMarks || "Not provided"}</p>
              <p className="text-gray-600"><span className="font-medium">Purchase Receipt:</span> {formData.itemIdentifiers.purchaseReceipt || "Not provided"}</p>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="ml-4 mt-1">
              <p className="text-gray-600"><span className="font-medium">Phone:</span> {formData.contactInformation.phone || "Not provided"}</p>
              <p className="text-gray-600"><span className="font-medium">Email:</span> {formData.contactInformation.email || "Not provided"}</p>
              <p className="text-gray-600"><span className="font-medium">Preferred Contact:</span> {formData.contactInformation.preferredContactMethod}</p>
            </div>
          </div>
          
          <div>
            <h4 className="text-md font-semibold text-gray-700">Delivery Preferences</h4>
            <div className="ml-4 mt-1">
              <p className="text-gray-600"><span className="font-medium">Meetup Method:</span> {formData.meetupPreference}</p>
              {formData.meetupPreference !== "inPerson" && (
                <>
                  <p className="text-gray-600"><span className="font-medium">Address:</span> {formData.deliveryAddress.streetAddress}</p>
                  <p className="text-gray-600">{formData.deliveryAddress.cityName}, {formData.deliveryAddress.stateOrProvince} {formData.deliveryAddress.postalCode}</p>
                  <p className="text-gray-600">{formData.deliveryAddress.countryName}</p>
                  {formData.deliveryAddress.addressNotes && (
                    <p className="text-gray-600"><span className="font-medium">Notes:</span> {formData.deliveryAddress.addressNotes}</p>
                  )}
                </>
              )}
            </div>
          ...
          <div>
            <h4 className="text-md font-semibold text-gray-700">Verification Method</h4>
            <p className="text-gray-600 mt-1">{formData.claimVerificationMethod}</p>
          </div>
          
          {formData.attachments.length > 0 && (
            <div>
              <h4 className="text-md font-semibold text-gray-700">Attached Files</h4>
          ...
          {formData.additionalNotes && (
            <div>
              <h4 className="text-md font-semibold text-gray-700">Additional Notes</h4>
        ...
        <div className="mt-6 text-sm text-gray-500">
          <p>By submitting this claim, you confirm that all information provided is accurate and truthful.</p>
        </div>
      </div>
    );
  };
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="hidden md:block">
              <div className="text-xs font-semibold inline-block py-1 px-2 uppercase rounded-full text-indigo-100 bg-indigo-700">
                Step {currentStep} of 3: {steps.find(step => step.id === currentStep)?.label}
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="md:w-64 bg-indigo-900 text-white p-6">
            ...
            <div className="mt-10 pt-6 border-t border-indigo-800">
              <div className="text-indigo-300 text-sm">
                ...
                <p className="text-xs opacity-80">Contact support for assistance with your claim request.</p>
